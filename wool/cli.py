...
async def run_repl(resume: bool = False) -> None:
    ...
    if config.mcp_servers:
        ...
        async def connect_mcp(name: str, mcp_cfg: dict):
            try:
                ...
                tools = agent.mcp_manager._clients[name].tools
    ...
    if not agent.messages:
        ...
        if config.active_session == path.stem:
            if config.last_session and agent.get_session_path(config.last_session).exists():
