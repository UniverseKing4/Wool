...
class SlashCommandHandler:
    ...
    async def _help(self, _args: str) -> bool:
        ...
        cmds = [
            ...
            ("/mcp(s) [list|connect|disconnect]", "Manage MCP servers (interactive menu if no args)"),
    ...
    async def _model(self, args: str) -> bool:
        ...
        if sub == "show":
            ...
            print()
            for m in models:
                active = " ◀" if m.id == active_model else ""
                print(f"  {cyan(m.id)}{active}")
            print()
    ...
    async def _resume(self, _args: str) -> bool:
        ...
        if not last:
            ansi_error("No previous session to resume.")
            return False
