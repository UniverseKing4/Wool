...
class StreamPrinter:
    ...
    def _print_line(self, line: str) -> None:
        ...
        if _HR_RE.match(line):
            sys.stdout.write(
                self._apply_style(f"  {_a(_DIM)}{'─' * 50}{_a(_RST)}") + "\r\n"
            )
            sys.stdout.flush()
            return
        ...
        if hm:
            ...
            sys.stdout.write(
                self._apply_style(f"  {_a(color)}{prefix}{rendered_text}{_a(_RST)}")
                + "\r\n"
            )
            if level <= 2:
                width = min(len(text_part) + 4, 50)
                sys.stdout.write(
                    self._apply_style(f"  {_a(_DIM)}{'─' * width}{_a(_RST)}") + "\r\n"
                )
            sys.stdout.flush()
            return
        ...
        if um:
            ...
            pad = "  " * depth
            sys.stdout.write(
                self._apply_style(f"  {pad}{_a(_FG_CYAN)}{bullet}{_a(_RST)} {content}")
                + "\r\n"
            )
            sys.stdout.flush()
            return
        ...
        if om:
            ...
            pad = "  " * depth
            sys.stdout.write(
                self._apply_style(f"  {pad}{_a(_FG_CYAN)}{num}.{_a(_RST)} {content}")
                + "\r\n"
            )
            sys.stdout.flush()
            return
